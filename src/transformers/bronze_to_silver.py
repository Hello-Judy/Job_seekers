...
# ---------------------------------------------------------------------------
# SQL: USAJobs Historical Bronze → Silver
# ---------------------------------------------------------------------------
# The historical parquet data is stored as JSON VARIANT in
# BRONZE.raw_usajobs_historical.  The extractor already normalised column
# names to our internal schema (position_id, job_title, agency, …) before
# serialising to JSON, so VARIANT paths here match those internal names.
#
# Salary is already annualised by the Python extractor; no PH/PB conversion
# needed here.  Experience level uses the same title-keyword + salary-min
# heuristics as the current USAJobs feed.

USAJOBS_HISTORICAL_MERGE_SQL = """
MERGE INTO SILVER.jobs_unified AS tgt
USING (
    SELECT
        'hist_' || position_id                           AS job_id,
        'usajobs_historical'                             AS source,
        'federal'                                        AS sector,

        raw_data:job_title::STRING                       AS job_title,
        TRIM(REGEXP_REPLACE(
            LOWER(raw_data:job_title::STRING),
            '[^a-zA-Z0-9]+', ' '))                       AS job_title_normalized,
        COALESCE(
            raw_data:agency::STRING,
            raw_data:department::STRING)                 AS company_or_agency,
        NULL                                             AS job_description,

        raw_data:city::STRING                            AS location_city,
        raw_data:state::STRING                           AS location_state,
        COALESCE(raw_data:country::STRING, 'US')         AS location_country,
        ...
        raw_data:job_category::STRING                    AS job_category,

        CASE
            WHEN REGEXP_LIKE(LOWER(raw_data:job_title::STRING),
                             '.*(intern|internship|entry[- ]level|junior|graduate|trainee).*')
                 THEN 'entry'
            WHEN REGEXP_LIKE(LOWER(raw_data:job_title::STRING),
                             '.*(senior|sr\\\\.|principal|staff|lead|director|chief|executive).*')
                 THEN 'senior'
            WHEN raw_data:salary_min::FLOAT < 55000
                 THEN 'entry'
            WHEN raw_data:salary_min::FLOAT >= 100000
                 THEN 'senior'
            ELSE 'mid'
        END                                              AS experience_level,

        REGEXP_LIKE(LOWER(raw_data:job_title::STRING),
                    '.*(intern|internship|entry[- ]level|junior|graduate|trainee).*')
                                                         AS is_entry_level,

        CASE
            WHEN LOWER(raw_data:schedule::STRING) LIKE '%part%' THEN 'part_time'
            WHEN REGEXP_LIKE(LOWER(raw_data:job_title::STRING), '.*intern.*') THEN 'intern'
            ELSE 'full_time'
        END                                              AS employment_type,

        CASE
            WHEN raw_data:remote::BOOLEAN = TRUE         THEN 'remote'
            ELSE 'onsite'
        END                                              AS remote_type,

        TRY_TO_DATE(raw_data:posted_date::STRING)        AS posted_date,
        TRY_TO_DATE(raw_data:close_date::STRING)         AS close_date,

        raw_data:apply_url::STRING                       AS apply_url,
...
class BronzeToSilverTransformer:
    ...
    def transform_usajobs_historical(self) -> int:
        """Upsert historical Bronze rows (raw_usajobs_historical) into Silver.

        Safe to call after every historical batch load — idempotent MERGE.
        Returns the number of rows affected.
        """
        logger.info("Running USAJobs Historical Bronze→Silver MERGE")
        with self._connect() as conn:
            cur = conn.cursor()
            try:
                cur.execute(USAJOBS_HISTORICAL_MERGE_SQL)
                affected = cur.rowcount
                conn.commit()
            finally:
                cur.close()
        logger.info("USAJobs Historical MERGE affected %s rows", affected)
        return affected or 0
