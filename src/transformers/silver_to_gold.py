...
USING (
    -- Wrap the join in an outer SELECT so we can apply QUALIFY for dedup.
    -- Even if dim_company / dim_location accumulate duplicates over time
    -- (e.g. due to whitespace or NULL edge cases in MERGE ON conditions),
    -- this guarantees one row per posting_key, preventing MERGE failures.
    SELECT * FROM (
        SELECT
        ...
        QUALIFY ROW_NUMBER() OVER (
            PARTITION BY ju.source || '::' || ju.job_id
            ORDER BY ju.ingestion_timestamp DESC
        ) = 1
    )
) AS src
